...
public class DragSpellOnTarget : DraggingActions {
    ...
    {
        tempVisualState = whereIsThisCard.EstadoVisual;
        whereIsThisCard.EstadoVisual = VisualStates.Arrastrando;
        sr.enabled = true;
        lr.enabled = true;
    }
    ...
    public override void OnEndDrag()
    {
        ...
        if (Target != null)
        {
            ...
            Player owner = null; 
            if (tag.Contains("Low"))
                owner = DatosGenerales.Instance.LowPlayer;
            else
                owner = DatosGenerales.Instance.TopPlayer;
            ...
            switch (Targets)
            {
                case TargetingOptions.AllCharacters: 
                    owner.JugarSpellMano(GetComponentInParent<IDHolder>().UniqueID, targetID);
                    targetValid = true;
                    break;
        ...
        if (!targetValid)
        {
            ...
            whereIsThisCard.EstadoVisual = tempVisualState;
            whereIsThisCard.SetearOrdenCarta();
        }
